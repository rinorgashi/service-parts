...
export default function Sales() {
    ...
    const [formData, setFormData] = useState({
        ...
        labour_cost: '0', guarantee_included: false, notes: ''
    });
    ...
    const fetchData = async () => {
        try {
            const [salesRes, partsRes, customersRes] = await Promise.all([
                fetch('/api/sales'), fetch('/api/parts'), fetch('/api/customers')
            ]);
    ...
    const openAddModal = () => {
        setFormData({ part_id: '', customer_id: '', quantity: '1', unit_price: '', labour_cost: '0', guarantee_included: false, notes: '' });
        setModalOpen(true);
    };

    const handlePartChange = (partId) => {
        const part = parts.find(p => p.id === parseInt(partId));
        setFormData({ ...formData, part_id: partId, unit_price: part ? part.selling_price.toString() : '' });
    };
        ...
        <>
                ...
                {sales.length === 0 ? (
                    ...
                    <div className="table-container">
                        <table className="table">
                            <thead><tr><th>Date</th><th>Part</th><th>Customer</th><th>Qty</th><th>Unit Price</th><th>Labour</th><th>Total</th><th>Guarantee</th><th>Actions</th></tr></thead>
                            <tbody>
                                {sales.map((s) => (
                                    <tr key={s.id}>
            ...
            <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="New Sale"
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    {selectedPart && selectedPart.quantity_in_stock < 10 && (
                        <div style={{ background: 'var(--warning-bg)', color: 'var(--warning)', padding: '12px', borderRadius: 'var(--radius-sm)', marginBottom: '16px', fontSize: '0.9rem' }}>
                            ⚠️ Only {selectedPart.quantity_in_stock} items left in stock
                        </div>
                    )}
                    ...
                    <div className="form-row">
                        <div className="form-group"><label className="form-label">Quantity *</label><input type="number" min="1" max={selectedPart?.quantity_in_stock || 999} className="form-input" value={formData.quantity} onChange={(e) => setFormData({ ...formData, quantity: e.target.value })} required /></div>
